...
class Particle(object):
	...
	xv,yv = odom_msg.twist.twist.linear.x,odom_msg.twist.twist.linear.y
	xv_noise =xv +  np.random.normal(0,LINEAR_MODEL_VAR_X)
	yv_noise =yv +  np.random.normal(0,LINEAR_MODEL_VAR_Y)
	P = np.array([xv_noise,yv_noise])

	w = odom_msg.twist.twist.angular.z		
	self.map_T_particle.rotation += w*dt

	P_r = RotateBy(P,self.map_T_particle.rotation)
	self.map_T_particle.translation += np.dot(P_r,dt)
...
class ParticleFilter(object):s
	...
	for i in range(0,len(self.particles)):
		nu+= np.float64(self.particles[i].weight)
	for i in range(0,len(self.particles)):
		ndp.append((np.float64(self.particles[i].weight)/nu))
