...
#==============  MUST IMPORT FOR SOLING EXTRA CREDIT 3 QUESTION =============================
...
class ParticleFilter(object):
	...
	for particle in self.particles:
    		...
    		xval = particle.map_T_particle.translation[0]
    		yval = particle.map_T_particle.translation[1]	
    		X.append(np.array([xval,yval]))
	#if(self.flag ==1):
        clustering = DBSCAN(eps=0.3, min_samples=1).fit(X)
        labels = clustering.labels_
        uniqueValues, occurCount = np.unique(labels, return_counts=True)
        index_of_max = np.where(occurCount == np.max(occurCount))[0][0]
        print("UNIQ  : ",uniqueValues)
        print("OCCRCNT  : ",occurCount)		
        print("INDEX MAX  : ",index_of_max)
        print("OCCCC:",occurCount[index_of_max],np.dtype(occurCount[index_of_max]))
        for i in range(0,len(labels)):
      		if(labels[i]==index_of_max):
			sum_translation += self.particles[i].map_T_particle.translation		
			sum_rotation += self.particles[i].map_T_particle.rotation
	avg_translation = sum_translation /occurCount[index_of_max] 
